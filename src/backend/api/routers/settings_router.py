...
async def get_picovoice_access_key():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve Picovoice access key: {str(e)}"
        )
...
async def set_picovoice_access_key(data: AccessKeyModel):
    ...
    try:
        ...
        return {
            "message": "Picovoice access key updated successfully",
            "key": result["key"],
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to update Picovoice access key: {str(e)}"
        )
