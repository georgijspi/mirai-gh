...
async def add_agent(
    agent_data: AgentCreate, current_user: dict = Depends(get_current_user)
):
    """Create a new agent."""
    try:
...
async def upload_profile_picture(agent_uid: str, file: UploadFile = File(...)):
...
async def upload_custom_voice(agent_uid: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        valid_file = (file.content_type in ALLOWED_AUDIO_TYPES) or (
            file.filename and file.filename.lower().endswith(".wav")
        )

        if not valid_file:
            logger.error(
                f"Invalid file format: {file.filename}, content_type: {file.content_type}"
            )
        ...
        logger.info(
            f"Custom voice successfully uploaded for agent {agent_uid}: {voice_path}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to upload custom voice: {str(e)}"
        )
...
async def get_agent_by_id(
    agent_uid: str, current_user: dict = Depends(get_current_user)
):
    """Get an agent by ID."""
    agent = await get_agent(agent_uid)
    if not agent:
...
async def get_profile_picture(
    ...
    try:
        ...
        if not agent.get("profile_picture_path"):
            raise HTTPException(
                status_code=404, detail="Agent does not have a profile picture"
            )
        ...
        if not os.path.exists(file_path):
            raise HTTPException(
                status_code=404, detail="Profile picture file not found"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve profile picture: {str(e)}"
        )
