...
async def broadcast_to_conversation(conversation_id: str, message: str):
    ...
    if conversation_id in conversation_connections:
        logger.info(f"Broadcasting message to {len(conversation_connections[conversation_id])} clients for conversation: {conversation_id}")
        disconnected = set()
        for connection in conversation_connections[conversation_id]:
            try:
                await connection.send_text(message)
                logger.debug(f"Message sent to client for conversation: {conversation_id}")
        ...
        for conn in disconnected:
            ...
            logger.info(f"Removed disconnected client from conversation {conversation_id}")
    else:
...
async def broadcast_to_global(message: str):
    ...
    logger.info(f"Broadcasting message to {len(active_connections['global'])} global clients")
    disconnected = set()
...
async def global_websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        logger.info(f"WebSocket connection accepted for /ws/global from {websocket.client}")
...
async def conversation_websocket_endpoint(websocket: WebSocket, conversation_id: str):
    ...
    logger.info(f"WebSocket connection attempt to /ws/conversation/{conversation_id} from {websocket.client}")
    
    try:
        await websocket.accept()
        logger.info(f"WebSocket connection accepted for /ws/conversation/{conversation_id} from {websocket.client}")
        ...
        try:
            ...
            if conversation_id in conversation_connections:
                ...
                logger.info(f"WebSocket disconnected from /ws/conversation/{conversation_id}: {websocket.client}")
