...
{tts_instructions}"""
    ...
    return PromptBuilder.create_prompt(
        ...
        messages=(
            recent_messages[-4:] if len(recent_messages) > 4 else recent_messages
        ),  # Use even fewer messages
...
async def get_global_conversation(
    limit: int = 50, skip: int = 0, current_user: dict = Depends(get_current_user)
):
    """Get the global conversation with recent messages."""
    try:
        conversation = await get_global_conversation_with_messages(
            limit=limit, skip=skip
        )
        logger.info(
            f"Retrieved global conversation with {len(conversation.get('messages', []))} messages"
        )
        return conversation
...
async def send_global_message(
    ...
    try:
        ...
        user_message = await add_message_to_global_conversation(
            content=message_data.content, message_type=MessageType.USER
        )
        ...
        if not agent_uid:
            ...
            matches = re.findall(
                agent_mention_pattern, message_data.content, re.IGNORECASE
            )
            ...
            if matches:
                ...
                for agent in agents:
                    ...
                    if mentioned_name in agent_name.split():
                        ...
                        logger.info(
                            f"Detected agent mention: {mentioned_name}, matching to agent: {agent['name']}"
                        )
                        break

        if not agent_uid:
            logger.info(
                "No agent specified or detected in message, not generating a response"
            )
            return user_message
        ...
        logger.info(
            f"Starting background task for global conversation with agent {agent_uid}"
        )
        ...
        logger.info(
            f"User message sent and agent response processing started for global conversation"
        )
        return user_message
    except Exception as e:
...
async def process_agent_global_response(
    user_message: str, agent_uid: str, start_time: float = None
):
    ...
    try:
        ...
        try:
            ...
            prompt_path = os.path.join(
                GLOBAL_CONVERSATION_DIR, f"prompt_{message_uid}.txt"
            )
        ...
        if "llm_config_uid" in llm_config_json:
            llm_config_id = (
                str(llm_config_json["llm_config_uid"])
                if isinstance(llm_config_json["llm_config_uid"], (ObjectId, str))
                else llm_config_json["llm_config_uid"]
            )
        elif "config_uid" in llm_config_json:
            llm_config_id = (
                str(llm_config_json["config_uid"])
                if isinstance(llm_config_json["config_uid"], (ObjectId, str))
                else llm_config_json["config_uid"]
            )

        # Generate a stream URL for the audio
        audio_stream_url = (
            f"/mirai/api/tts/stream/{message_uid}?conversation_uid=global"
        )
        ...
        logger.error(
            f"Error processing agent response for global conversation: {str(e)}"
        )
        import traceback
...
async def rate_global_message(
    ...
    try:
        ...
        updated_message = await update_global_message_rating(
        ...
        logger.info(
            f"Global message {rating_data.message_uid} rated as {rating_data.rating}"
        )
