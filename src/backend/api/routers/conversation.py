...
async def create_new_conversation(
    conv_data: ConversationCreate, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        user_uid = (
            current_user.get("user_uid")
            if isinstance(current_user, dict)
            else current_user.user_uid
        )
        ...
        title = (
            conv_data.title if hasattr(conv_data, "title") and conv_data.title else None
        )

        conversation = await create_conversation(
            user_uid=user_uid, title=title, agent_uid=agent_uid
        )
...
async def get_conversation_detail(
    conversation_uid: str, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        if conversation["user_uid"] != user_uid:
            logger.warning(
                f"User {user_uid} tried to access conversation {conversation_uid} belonging to {conversation['user_uid']}"
            )
            raise HTTPException(
...
async def update_conversation_details(
    ...
    try:
        ...
        if conversation["user_uid"] != user_uid:
            logger.warning(
                f"User {user_uid} tried to update conversation {conversation_uid} belonging to {conversation['user_uid']}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def archive_conversation_by_id(
    ...
    try:
        ...
        if conversation["user_uid"] != user_uid:
            logger.warning(
                f"User {user_uid} tried to archive conversation {conversation_uid} belonging to {conversation['user_uid']}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def send_message(
    ...
    try:
        ...
        logger.info(
            f"Received message request for conversation: {message_data.conversation_uid}"
        )
        ...
        logger.info(
            f"Found conversation in DB with uid: {conversation.get('conversation_uid')}"
        )

        # Check if the conversation belongs to the current user
        if conversation["user_uid"] != user_uid:
            logger.warning(
                f"User {user_uid} tried to send message to conversation {message_data.conversation_uid} belonging to {conversation['user_uid']}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if not agent:
            logger.error(
                f"Agent not found for conversation: {message_data.conversation_uid}"
            )
            raise HTTPException(
        ...
        logger.info(
            f"User message sent and agent response processing started for conversation: {conversation_uid}"
        )
        return user_message
    except HTTPException:
        raise
...
async def process_agent_response(
    ...
    try:
        ...
        rag_result = await augment_conversation_context(
            conversation_messages_json, user_message
        )
        rag_context = None
        api_module_context = None

        if rag_result.get("api_module_applied", False) and rag_result["system_message"]:
            api_module_context = rag_result["system_message"]["content"]
            logger.info(f"API module applied to query: {user_message}")

            metadata = {
                "api_module_applied": True,
                "module_name": rag_result.get("api_module_result", {}).get(
                    "module_name", "Unknown"
                ),
                "execution_time": rag_result.get("api_module_result", {}).get(
                    "execution_time", 0
                ),
            }

            # Update the user message with API module metadata
            try:
                message_uid = None
                # Find the most recent user message with matching content
                for message in reversed(conversation_messages_json):
                    if (
                        message.get("message_type") == MessageType.USER
                ...
                if message_uid and db is not None:
                    await db[MESSAGE_COLLECTION].update_one(
                        {"message_uid": message_uid}, {"$set": {"metadata": metadata}}
                    )
                    logger.info(
                        f"Updated message {message_uid} with API module metadata"
                    )
                else:
                    logger.warning(
                        f"Could not update API module metadata: "
                        + (
                ...
                logger.warning(
                    f"Failed to update message with API module metadata: {str(e)}"
                )
        elif rag_result["rag_applied"] and rag_result["system_message"]:
        ...
        prompt = build_prompt(
            ...
            agent_config,
            tts_instructions,
            rag_context if rag_context else api_module_context,
        )
        ...
        logger.info(
            f"Generating voice for message_uid: {message_uid} in conversation: {conversation_uid}"
        )
        ...
        if start_time:
            ...
            logger.info(
                f"Total response time (including TTS): {response_time:.2f} seconds"
            )

        # Generate a stream URL for the audio
        audio_stream_url = (
            f"/mirai/api/tts/stream/{message_uid}?conversation_uid={conversation_uid}"
        )
        ...
        if db is not None:
            try:
                ...
                await db[MESSAGE_COLLECTION].update_one(
                    {"message_uid": message_uid}, {"$set": update_fields}
                )
                logger.info(
                    f"Updated message {message_uid} with audio_stream_url and prompt_path in database"
                )
            except Exception as e:
            ...
            logger.warning(
                f"Database connection not available, couldn't update message {message_uid} with audio_stream_url and prompt_path"
            )
        ...
        if pubsub_client:
            try:
                ...
                logger.info(
                    f"Published agent response for conversation: {conversation_uid}"
                )
            except Exception as e:
...
async def rate_message(
    rating_data: RateMessageRequest, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        if conversation["user_uid"] != user_uid:
            logger.warning(
                f"User {user_uid} tried to rate message in conversation belonging to {conversation['user_uid']}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        updated_message = await update_message_rating(
            rating_data.message_uid, rating_data.rating
        )
        if not updated_message:
            logger.warning(
                f"Failed to update message rating: {rating_data.message_uid}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
