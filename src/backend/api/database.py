...
class PubSub:
    ...
    async def publish(self, channel: str, message: str):
        ...
        if len(self.message_buffer[channel]) > self.buffer_limit:
            self.message_buffer[channel] = self.message_buffer[channel][-self.buffer_limit:]
    ...
    def subscribe(self, channel: str, callback: Callable):
        ...
        logger.info(f"Added subscriber to channel: {channel}, total subscribers: {len(self.subscribers[channel])}")
    ...
    def unsubscribe(self, channel: str, callback: Callable):
        ...
        if channel in self.subscribers and callback in self.subscribers[channel]:
            self.subscribers[channel].remove(callback)
            logger.info(f"Removed subscriber from channel: {channel}, remaining subscribers: {len(self.subscribers[channel])}")
            return True
        return False
