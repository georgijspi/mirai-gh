...
async def create_conversation(
    user_uid: str, title: str = None, agent_uid: str = None
) -> Dict[str, Any]:
    ...
    logger.info(
        f"Created new conversation in database: {title} with UID: {conversation_uid}"
    )
...
async def get_conversation(conversation_uid: str) -> Optional[Dict[str, Any]]:
    ...
    db = get_database()
    conversation = await db[CONVERSATION_COLLECTION].find_one(
        {"conversation_uid": conversation_uid}
    )
    return conversation
...
async def get_conversation_with_messages(
    ...
    cursor = (
        ...
        .find({"conversation_uid": conversation_uid})
        .sort("created_at", 1)
    )
...
async def get_user_conversations(
    user_uid: str, include_archived: bool = False
) -> List[Dict[str, Any]]:
...
async def update_conversation(
    conversation_uid: str, update_data: Dict[str, Any]
) -> Optional[Dict[str, Any]]:
    """Update conversation."""
    db = get_database()
    ...
    result = await db[CONVERSATION_COLLECTION].update_one(
        {"conversation_uid": conversation_uid}, {"$set": update_data}
    )
...
async def add_message_to_conversation(
    conversation_uid: str, content: str, is_user: bool = True
) -> Optional[Dict[str, Any]]:
    """
    ...
    if not conversation:
        logger.error(
            f"Failed to add message: Conversation {conversation_uid} not found"
        )
        return None
    ...
    if is_user:
        ...
        if not agent:
            logger.error(
                f"Failed to generate response: Agent {conversation['agent_uid']} not found"
            )
            return message

        response_content = await generate_text(
            conversation_uid=conversation_uid, agent=agent, user_message=content
        )
        ...
        if response_content:
            ...
            if response_message:
                ...
                if agent.get("custom_voice_path") and os.path.exists(
...
async def update_message_rating(
    ...
    result = await db[MESSAGE_COLLECTION].update_one(
        {"message_uid": message_uid}, {"$set": {"rating": rating}}
    )
...
async def add_message(
    ...
    logger.info(
        f"Inserted message {message_uid} into database for conversation {conversation_uid}"
    )
    ...
    logger.info(
        f"Added {message_type} message {message_uid} to conversation {conversation_uid}"
    )
    return message_data
