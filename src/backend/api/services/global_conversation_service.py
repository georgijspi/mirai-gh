...
async def get_or_create_global_conversation() -> Dict[str, Any]:
    ...
    global_conv = await db[GLOBAL_CONVERSATION_COLLECTION].find_one({"conversation_uid": GLOBAL_CONVERSATION_ID})
...
async def get_global_conversation_with_messages(limit: int = 50, skip: int = 0) -> Dict[str, Any]:
    """Get the global conversation with messages, with pagination."""
    db = get_database()
    
    global_conv = await get_or_create_global_conversation()
    
    cursor = db[GLOBAL_MESSAGE_COLLECTION].find(
        {"conversation_uid": GLOBAL_CONVERSATION_ID}
...
async def update_global_message_rating(message_uid: str, rating: MessageRating) -> Optional[Dict[str, Any]]:
