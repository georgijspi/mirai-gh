...
async def process_query_with_rag(
    query: str, conversation_uid: str = None
) -> Dict[str, Any]:
    """Process a user query with RAG if needed."""
    logger.info(f"Processing query with potential RAG: {query}")

    # First, check if the query matches any API module
    api_match = await check_api_module_match(query)
    if api_match["matched"]:
        logger.info(f"Query matched API module {api_match['module_name']}")

        # Create a result with API module data
        result = {
            "original_query": query,
            "query_type": QueryType.TRIVIA,  # API queries are treated as factual/trivia
            "enhanced_prompt": query,
            "search_context": "",
            "search_results": [],
            "using_rag": False,
            "using_api_module": True,
            "api_module_result": api_match,
        }
    ...
    if conversation_uid and conversation_uid in PREVIOUS_SEARCH_TERMS:
        ...
        logger.info(
            f"Found previous search terms for conversation {conversation_uid}: {prev_search_terms}"
        )

    # For corrections, skip full analysis if we have previous search terms
    if is_correction and prev_search_terms:
        logger.info(
            f"Detected correction message, using previous search terms: {prev_search_terms}"
        )
        ...
        if search_results:
            ...
            if len(search_context) > CONTEXT_LENGTH_LIMIT:
                search_context = (
                    search_context[:CONTEXT_LENGTH_LIMIT]
                    + "...\n(Search results truncated due to length)"
                )

            result.update(
                {"search_context": search_context, "search_results": search_results}
            )

            logger.info(
                f"Enhanced prompt created with {len(search_results)} search results for correction"
            )
        else:
    ...
    if query_type == QueryType.TRIVIA:
        ...
        if search_results:
            ...
            logger.info(
                f"Enhanced prompt created with {len(search_results)} search results"
            )
        else:
...
async def augment_conversation_context(
    conversation_messages: List[Dict[str, Any]], current_message: str
) -> Dict[str, Any]:
    ...
    # Check if this matched an API module
    if rag_result.get("using_api_module", False) and rag_result.get(
        "api_module_result", {}
    ).get("matched", False):
        logger.info("Adding API module context to conversation")

        api_result = rag_result["api_module_result"]
        module_name = api_result["module_name"]
        formatted_response = (
            api_result["formatted_response"] or "No formatted response available."
        )

        # Create the system message with API module context
        system_message = {
            "role": "system",
            "content": f"""API MODULE RESULT:

The user's query "{current_message}" was processed by the API module "{module_name}".

Result:
{formatted_response}

INSTRUCTIONS:
1. Inform the user that their query was processed by an external service
2. Present the information from the API module in a natural, conversational way
3. Stay in character while providing the information
4. Do not mention "API module" specifically - say something like "I checked that for you" or "I found this information"
5. Format the information in a readable way if needed
6. Do not include technical details about the API call itself
""",
        }

        # Return the augmented context with API system message
        return {
            "rag_applied": False,
            "api_module_applied": True,
            "system_message": system_message,
            "query_type": rag_result["query_type"],
            "api_module_result": api_result,
        }
...
Note: If the user is correcting previously provided information, acknowledge the correction and provide the accurate information.""",
            ...
            is_follow_up = any(
                ...
                for word in follow_up_indicators + previous_context_references
            )
            ...
            if is_follow_up and len(conversation_messages) >= 2:
                ...
                for i in range(len(conversation_messages) - 1, 0, -1):
                    if (
                        conversation_messages[i]["message_type"] == "agent"
                        and i > 0
                        and conversation_messages[i - 1]["message_type"] == "user"
                    ):
