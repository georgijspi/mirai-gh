...
async def update_llm_config(
    config_uid: str, update_data: Dict[str, Any]
    ...
    result = await db[LLM_CONFIG_COLLECTION].update_one(
        {"config_uid": config_uid}, {"$set": update_data}
    )
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{OLLAMA_API_BASE}/pull", json={"name": model_name}
            )
            response.raise_for_status()
            return {
                "status": "success",
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.request(
                "DELETE", f"{OLLAMA_API_BASE}/delete", content=data, headers=headers
            )
            response.raise_for_status()
            return {
                "status": "success",
...
async def generate_text(
    ...
    **kwargs,
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{OLLAMA_API_BASE}/generate", json=params, timeout=90.0
            )
            response.raise_for_status()
            data = response.json()
