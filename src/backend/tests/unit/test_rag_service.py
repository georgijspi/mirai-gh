...
async def test_process_query_with_api_module(mock_api_match):
    ...
    assert (
        result["query_type"] == QueryType.TRIVIA
    )  # API module queries are treated as trivia
    assert result["using_rag"] is False
...
async def test_augment_conversation_context_no_rag(
    mock_process_query, sample_conversation_messages
):
    ...
    result = await augment_conversation_context(
        sample_conversation_messages, "How are you feeling today?"
    )
...
@pytest.mark.asyncio
@patch("api.services.rag_service.process_query_with_rag")
async def test_augment_conversation_context_with_rag(
    mock_process_query, sample_conversation_messages
):
    ...
    result = await augment_conversation_context(
        sample_conversation_messages, "What is the population of Ireland?"
    )
...
@pytest.mark.asyncio
@patch("api.services.rag_service.process_query_with_rag")
async def test_augment_conversation_context_with_api_module(
    mock_process_query, sample_conversation_messages
):
    ...
    result = await augment_conversation_context(
        sample_conversation_messages, "What's the weather like in Dublin today?"
    )
