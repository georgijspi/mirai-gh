...
def mock_db():
    ...
    mock_db = {"conversations": mock_collection, "messages": AsyncMock()}
    return mock_db
...
async def test_update_conversation(mock_get_db, mock_db, sample_conversation):
    ...
    with patch(
        "api.services.conversation_service.get_conversation"
...
async def test_get_user_conversations(mock_get_db, mock_db):
    ...
    mock_get_db.return_value = mock_db
    conversations = [{"title": "Conversation 1"}, {"title": "Conversation 2"}]
...
async def test_add_message_to_conversation(
    ...
    with patch("uuid.uuid4") as mock_uuid:
        ...
        with patch("os.path.exists") as mock_path_exists:
            ...
            # Mock get_message
            with patch(
                "api.services.conversation_service.get_message"
            ) as mock_get_message:
                mock_get_message.side_effect = lambda mid: (
                    expected_user_message
                    if mid == user_message_id
                    else expected_agent_message
                )
                ...
                assert (
                    first_call_args["conversation_uid"]
                    == sample_conversation["conversation_uid"]
                )
                ...
                second_call_args = mock_db["messages"].insert_one.call_args_list[1][0][
                    0
                ]
                assert (
                    second_call_args["conversation_uid"]
                    == sample_conversation["conversation_uid"]
                )
...
async def test_update_message_rating(mock_get_db, mock_db, sample_message):
    ...
    with patch("api.services.conversation_service.get_message") as mock_get_message:
        ...
        result = await update_message_rating(
            message_uid=sample_message["message_uid"], rating=MessageRating.LIKE
        )
...
async def test_get_conversation_with_messages(
    mock_get_db, mock_db, sample_conversation
):
